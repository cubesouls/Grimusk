...
namespace Grimusk
{
    class Processor
    {
        ...
        public static void Process() // sempre vai processar a instruçao no opcode
        {
            ...
            IR = Memory.instructions[pc].opcode;
            IAS = Memory.instructions[pc];
        ...
        {
            List<string> log = new List<string>(); // serve para mostrar tudo o que fizemos
            short AC;
            ...
            switch (opcode)
            {
                case -1: // opcode -1 para parar o programa
                    log.Add("O programa foi parado");
                    break;
                case 0: // opcode 0 limpa o MQ
                    MQ = 0;
                    log.Add("MQ voltou para -> 0");
                    break;
                case 1: // opcode 1 coloca MQ no addr 1 (addr[0])
                    Memory.data[instruction.addr[0]] = MQ;
                    log.Add("O valor de MQ foi guardado em ADDR1 -> [" + instruction.addr[0].ToString() + "]");
                    break;
                case 2: // opcode 2 busca o que ta no addr1, coloca no AC, soma com o que ta no addr2 e coloca no MQ (eu sei que tinha que dividir mais,
                    ...
                    log.Add("AC se tornou ADDR1 -> " + AC.ToString() + ", somando com ADDR2 -> " + instruction.addr[1].ToString());
                    MQ = (short)(AC + instruction.addr[1]);
                    log.Add("MQ se tornou -> " + MQ.ToString());
                    break;
                case 3: // opcode 3 coloca addr1 no AC, subtrai com addr2 e coloca no MQ
                    AC = instruction.addr[0];
                    log.Add("AC se tornou ADDR1 -> " + AC.ToString() + ", subtraindo com ADDR2 -> " + instruction.addr[1].ToString());
                    MQ = (short)(AC - instruction.addr[1]);
                    log.Add("MQ se tornou -> " + MQ.ToString());
                    break;
                case 4: // opcode 4 multiplica addr1 (addr2)vezes e coloca no MQ
                    AC = instruction.addr[1];
                    log.Add("AC se tornou -> " + AC.ToString());
                    while (AC > 0)
                    {
                        log.Add("AC ainda é maior que 0 (" + AC.ToString() + "), somando MQ novamente e diminuindo AC...");
            ...
            {
                Program.window.Log.Items.Add(log[x]);
            }
        }
    }
}
